! function(root, factory) {
    ...
    multiStep.prototype = {
        ...
        _buildModal: function() {
            ...
            this.element.html(`<div class="modal-dialog modal-dialog-centered modal-${this.options.modalSize} multi-step" style="height:90%;min-width:80%;background:#fff;" role="document"><div class="modal-content" style="height:100%;"><div class="modal-header"><h5 class="modal-title" id="${id}LongTitle">${this.options.title}</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button></div><div class="modal-body" style="overflow-y: scroll;"></div><div class="modal-footer"></div></div></div>`);
            var $this = this;
        ...
        _buildFooterContent: function() {
            ...
            <button type="button" class="btn btn-sm ${_skipClass}">ffff${this.options.skipText}</button>
        ...
        _attachNextEvent: function() {
            ...
            $this.next.click(function() {
				...
				var nextIdx   = $this.currentStepIdx;
				var data_step = "paso"+nextIdx;

				if((Object.entries(master[data_step]).length === 0) || (data_step <= 3)){
				  alert('Por favor elige la(s) opciones para avanzar');
				}
				else
				{
