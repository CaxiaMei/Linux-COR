...
class Usuario_model extends CI_Model {                
    ...
    public function get_usuario_by_id($identificador)
    {         
    ...
    public function relacion_usuario($identificador)
    {
    ...
    public function referencias_usuario($identificador)
    {
    ...
    public function get_direcciones_by_id($id_user)
    {
   
    $this->db->select("*");  
    $this->db->from  ("direcciones as d");
    $this->db->join  ("municipio as m", "d.id_municipio = m.id_municipio", "inner");
    $this->db->join  ("colonia as c", "d.id_colonia_dir = c.id_colonia", "inner");
    $this->db->where ("d.id_persona", $id_user);    
...
//    public function get_persona_by_afiliado($id_user)
//    {
//   
//    $this->db->select("*");  
...
//	busqueda
	...
	{//id_personaU,id_persona,,apellidos
	...
	$arr_afi='';
	 ...
	 foreach ($query as $row) {
		$arr_afi = $arr_afi."'".$row->nombre." ".$row->apellidos."',"; 
	 }
		
	 $arr_afi = trim($arr_afi, ',');
     return $arr_afi;
		
	}
	
	public function get_busqueda_complete()
	{
	 	
	 $this->db->select("id_personaU, CONCAT_WS(' ', nombre, apellidos ) as nombrec");  
     $this->db->from  ("persona");
	 $this->db->where ("tipo_de_persona", 2); 
	 $this->db->where ("status", 3); 
